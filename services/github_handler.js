...
export default function(req, res, next) {
  ...
  function publishToRabbitMQ(project) {
    ...
    rabbitmqService
      .publish(project.routing_key, jsonPayload)
      .then(function() {
        logger.logMessagePublished(
  ...
  function verifySignature(signature, secret, data) {
    ...
    return crypto.timingSafeEqual(
      ...
      new Buffer(computedSignature)
    );
  }

  if (
    verifySignature(req.get('X-Hub-Signature'), SECRET, JSON.stringify(payload))
  ) {
