...
function logInsufficientDataEvent(repoName) {
  console.warn(
    `[*] [${timestamp()}] Event came from repo '${repoName}' without the commit details. It may be first event from repo. Push something to repo.`
  );
}

function logMessagePublished(data, routingKey) {
  console.log(
    `[${data.id}] [${timestamp()}] Published to RabbitMQ, repo:${
      data.repository.name
    }, commit:${data.commit.id}, routing_key:${routingKey}`
  );
}

function logPublishError(data, error, routingKey) {
  console.error(
    `[${data.id}] [${timestamp()}] Error publishing to RabbitMQ, repo:${
      data.repository.name
    }, commit:${data.commit.id}, routing_key:${routingKey}`
  );
  console.error('\t', error);
...
function logInvalidSignature(repoName) {
  console.error(
    `[*] [${timestamp()}] Unauthorised event from repo'${repoName}'`
  );
}
